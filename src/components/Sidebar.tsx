...
export const Sidebar: React.FC = () => {
    ...
    const checkDepartmentAccess = async () => {
      ...
      try {
        ...
        const [departments, organizations] = await Promise.all([
          departmentsAPI.getByClassification('department'),
          departmentsAPI.getByClassification('organization')
        ]);
  ...
  if (!departmentLoading && hasDepartmentAccess) {
    navigation.push({ name: 'Departments', href: '/departments', icon: Building });
  }
  
  if (!departmentLoading && hasOrganizationAccess) {
