...
router.get('/settings/branding', async (req, res) => {
  try {
    const [logoSetting, communityNameSetting] = await Promise.all([
      getQuery(
    ...
    res.json({
      ...
      community_name: communityNameSetting?.setting_value || 'Lynus RP Hub'
    });
  } catch (error) {
...
router.put('/settings/branding', requireAdmin, async (req, res) => {
  try {
    const { custom_logo_url } = req.body;
    
    // Upsert the logo URL setting
    await runQuery(`
